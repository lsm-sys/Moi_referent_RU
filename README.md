...
- **Иллюстрация** — промпт через OpenRouter + изображение через Hugging Face (асинхронно)

## Кэш и предпросмотр

- Статья по URL кэшируется на **20 минут** — повторные действия не качают страницу заново.
- После ввода URL автоматически показывается **предпросмотр** (заголовок, дата, первые 500 символов).
- Иллюстрация генерируется **асинхронно**: `POST /api/illustrate` → опрос статуса каждые 2 с.
